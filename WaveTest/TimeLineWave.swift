...
struct Home: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            // wave from view...
            WaveFrom(color: .cyan.opacity(0.3), amplify: 150, isReversed: false)
                
            WaveFrom(color: (toggle ? Color.purple : Color.cyan).opacity(0.4), amplify: 100, isReversed: false)
...
//                context.draw(Text("\(offset)"), at: CGPoint(x: size.width / 2, y: 100))
    ...
    func  getPath(size: CGSize) ->Path{
        return Path{path in
            ...
            // for bottom
            path.addCurve(to: CGPoint(x: width, y: midHeight), control1: CGPoint(x: width * 0.35, y: midHeight +  amplify), control2: CGPoint(x: width * 0.65, y: midHeight - amplify))
